...
//	------ Setting up PWM1 using ePWM1A ------------------
	EPwm1Regs.TBCTL.bit.CTRMODE = TB_FREEZE;	// stop the counter
	EPwm1Regs.TBCTR = 0;						// counter is set zero
//	EPwm1Regs.TBPRD = 600;						// Period is set	
	EPwm1Regs.TBCTL.bit.CTRMODE = TB_COUNT_UPDOWN; 	// symmetrical
...
//	------- Setting up PWM3 using ePWM2A ------------------
	EPwm2Regs.TBCTL.bit.CTRMODE = TB_FREEZE;		// stop the counter
	EPwm2Regs.TBCTR = 0;						// counter is set zero
//	EPwm2Regs.TBPRD = 600;						// period is set, this will change depending on Mark & Set
	EPwm2Regs.TBCTL.bit.CTRMODE = TB_COUNT_UPDOWN;		// symmetrical count
...
//	-------- Setting the time base for the ADC sampling using ePWM3 TB --------
	EPwm3Regs.TBCTL.bit.CTRMODE = TB_FREEZE;		// stop the counter
	EPwm3Regs.TBCTR = 0;
	EPwm3Regs.TBPRD = RX_Sampling_TPR;		// this will be the ADC sampling period
	EPwm3Regs.TBCTL.bit.CTRMODE = 	TB_COUNT_UP;
	EPwm3Regs.ETPS.bit.SOCAPRD = ET_1ST;
	EPwm3Regs.ETSEL.bit.SOCAEN = 1;
	EPwm3Regs.ETSEL.bit.SOCASEL = ET_CTR_PRD; //ADC will be started at the period
	EPwm3Regs.TBCTL.bit.HSPCLKDIV = TB_DIV1;
	EPwm3Regs.TBCTL.bit.CLKDIV = TB_DIV1;
	
//  -------- Setting the PWM6 for the white LED --------------------------
	EPwm6Regs.TBCTL.bit.CTRMODE = TB_FREEZE;		// stop the counter
	EPwm6Regs.TBCTR = 0;
...
// This portion is taken to main.h by Arefeen on 062205 **** starts here ****
/*
	// ----- Set up T1PR -----
	#define Fbit		1.44				// arefeen
	#define TX_FREQ_s	(2500000/33.783)	// SET transmit frequency, 74kHz, arefeen
	#define TX_FREQ_m   (2500000/39.494)    // MARK transmit frequency, 63.3kHz, arefeen
	#define RX_Sampling	(TX_FREQ_m - 3*Fbit) // System sampling rate 58.98 kHz
	#define	TX_FREQ		(2500000/19) 		// Echelon transmit frequency = 131.57894 kHz
	
	#define TX_TPR_s    (((DSP_FREQ/TX_FREQ_s) + 1)/2)		// Transmit setting for SET, arefeen
	#define TX_TPR_m    (((DSP_FREQ/TX_FREQ_m) + 1)/2)		// Transmit setting for MARK, arefeen
	...
	#define RX_Sampling_TPR	(RX_TX_DRIFT+(DSP_FREQ/RX_Sampling)) //arefeen

*/
// This posrtion is taken to main.h by Arefeen on 062205 **** ends here ****

//	The next line is added by Arefeen on 072305
	EPwm1Regs.TBPRD = TX_TPR_s;
	EPwm2Regs.TBPRD = TX_TPR_s;
	
	
//	The next line is added by Arefeen on 062205	
//	EvaRegs.T1PR = TX_TPR_s;
...
// The next line is added by Arefeen on 062205
//	EvaRegs.T2PR = RX_Sampling_TPR-1;		// Set timer 2 period register to receive frequency	


// The next line is added by Arefeen on 072305
	EPwm3Regs.TBPRD = RX_Sampling_TPR-1;		// Set timer 2 period register to receive frequency	

	EPwm6Regs.TBPRD = 1U<<DAC_SIZE;		// Set timer period for white LED
	EPwm6Regs.TBCTL.bit.CTRMODE = TB_COUNT_UPDOWN;
...
// The next line is commented by Arefeen on 062205
//    EvaRegs.COMCONA.bit.ACTRLD = LD_IMMEDIATE;	// 11:10  	Action control register reload
// The next line is added by Arefeen on 062205
...
//	EvbRegs.COMCONB.bit.FCOMPOE = 1;      		// 9      	Full Compare output enable
  //  EvbRegs.COMCONB.bit.ACTRLD = LD_IMMEDIATE;	// 11:10  	Action control register reload
...
//	EvbRegs.T4CON.all = 0;
//	EvbRegs.T4CNT = 0;
//	EvbRegs.T4PR = TX_TPR_s;
//	EvbRegs.T4CON.bit.TCLKS10 = TCLKS_INT;		// 5:4   Clock source select = Internal (HSPCLK)
//	EvbRegs.T4CON.bit.TPS = TPS_DIV1;			// 10:8  Input clock prescaler = HSPCLK/1
//	EvbRegs.T4CON.bit.TMODE = TMODE_CONT_UPDOWN;// 12:11 Count mode selection = continuous up/down
