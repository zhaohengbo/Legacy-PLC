...
#define FIND_WORDSYNC_TO	34			// timeout after N bit times
...
//	#if AGC_ENABLE == True // commented by Arefeen 062205
	#if AGC_ENABLE == False
	...
	if( detBit == 1 )							// detBit is pos
	{
		if( phcos < (-PLL_VHYST_THRS) )			// phcos is neg
		{
			...
			if( pll.hystCnt > PLL_THYST_THRS )
			{
				...
				bitTransition = True;
//				if( pll.bitPhase < 160 ) // commented by arefeen 062205
//					pll.cycle = 0; // commented by arefeen 062205
			}
		}
	}
	else										// detBit is neg
...
//					pll.cycle = 0;
	...
	case FIND_BITSYNC:
		...
		if( bitTransition )
		{
			...
			uRxModeCount = 0;
			diagSample = 1;
//			pll.phaseHold = False;	// make sure PLL is running //are
