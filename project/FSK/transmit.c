...
s16		array_length = 0; //arefeen
s16		ADCsample_psk[14]; //arefeen
s16		ADCsample_psk_mul[14]; //arefeen
s16		array_delay = 12; //arefeen
s32		sum_array = 0;
s16		length	= 0;
s16		first_loop = 1;
s16		test = 1;
...
//		This line is commented by Arefeen on 062205			
//		receive(ADCsample[0]);
		
		if(first_loop == 0)
		{
		save = ADCsample_psk[array_length];
		}
		
//		ADCsample_psk[array_length] = test; //arefeen		
		ADCsample_psk[array_length] = ADCsample[0]; //arefeen
		ADCsample_psk_mul[array_length] = ADCsample_psk[array_length]*ADCsample_psk[array_delay];
//		test = test+1;

		if(first_loop == 1)
		{ sum_array = sum_array + ADCsample_psk_mul[array_length];}
//		{ sum_array = sum_array + ADCsample_psk[array_length];}
		...
		{ 
			...
			if(length > 13)
			{length = 0;}
//			what = ADCsample_psk[array_length];
			sum_array = sum_array + ADCsample_psk[array_length] - save;
		}
