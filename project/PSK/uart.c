...
void InitSci(void)
{
	...
	SciaRegs.SCIFFTX.bit.TXFIFORESET = 1;	// Enable receive FIFO operation.
...
void HandleUART(void)
{
	...
	if(SciaRegs.SCIRXST.bit.RXERR == 1)	// Receiver Error.
	{
		InitSci();		// Reset port.
	}
	...
	if (SciaRegs.SCIFFRX.bit.RXFIFST < ALMOST_FULL)
